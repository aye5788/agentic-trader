...
# OHLC price cache. NON-REGENERABLE: the deep panel is Schwab-era and that feed is
# gone (moomoo history is capped and shallow), so the mirror is the backup of
# record for it. A few MB. It used to also feed an off-box adaptive tuner; that
# layer was removed 2026-09-09 and the panel is backed up for its own sake.
