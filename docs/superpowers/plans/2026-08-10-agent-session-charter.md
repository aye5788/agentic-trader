...
Create `deploy/session_tools.sh` (it does not exist yet — Task 1 deliberately did
NOT create it, because the legacy loops must keep their Bash):
...
- [ ] **Step 5b: Verify it assembles**

Run: `source deploy/session_tools.sh && echo "${#SESSION_TOOL_ARGS[@]} args" && printf '%s\n' "${SESSION_TOOL_ARGS[@]}" | grep -c "^mcp__"`
Expected: at least 39 mcp__ entries (29 agentic + 10 Robinhood). If the count is lower, discovery under-reported — investigate before continuing.
